...
hsa_status_t aql_callback(const hsa_aql_trace_t *aql_trace, void *user_arg)
{
    switch (aql_trace->type) {
        case HSA_PACKET_TYPE_VENDOR_SPECIFIC:
            tracepoint(hsa_runtime, aql_packet_submitted, "queue_profiling", aql_trace->packet_id,
                "vendor_specific", aql_trace->agent.handle, aql_trace->queue->id);break;
        case HSA_PACKET_TYPE_INVALID:
            tracepoint(hsa_runtime, aql_packet_submitted, "queue_profiling", aql_trace->packet_id,
                "invalid", aql_trace->agent.handle, aql_trace->queue->id);break;
        case HSA_PACKET_TYPE_BARRIER_AND:
            tracepoint(hsa_runtime, aql_packet_submitted, "queue_profiling", aql_trace->packet_id,
                "barrier_and", aql_trace->agent.handle, aql_trace->queue->id);break;
        case HSA_PACKET_TYPE_AGENT_DISPATCH:
            tracepoint(hsa_runtime, aql_packet_submitted, "queue_profiling", aql_trace->packet_id,
                "agent_dispatch", aql_trace->agent.handle, aql_trace->queue->id);break;
        case HSA_PACKET_TYPE_BARRIER_OR:
            tracepoint(hsa_runtime, aql_packet_submitted, "queue_profiling", aql_trace->packet_id,
                "barrier_or", aql_trace->agent.handle, aql_trace->queue->id);break;
        case HSA_PACKET_TYPE_KERNEL_DISPATCH: {
            uint64_t kernel_object = ((hsa_kernel_dispatch_packet_t*) aql_trace->packet)->kernel_object;
            std::string name = symbol_names[kernel_object_symbols[kernel_object]];
            std::cout << "kname " << name << std::endl;
            tracepoint(hsa_runtime, aql_kernel_dispatch_packet_submitted,
                "queue_profiling", aql_trace->packet_id, aql_trace->agent.handle, aql_trace->queue->id, kernel_object, name.c_str());
        } break;
        default:
            break;
    }
    ...
    uint32_t group_segment_size, hsa_queue_t **queue)
{
    ...
    tracepoint(hsa_runtime, queue_created, "queue_profiling", agent.handle, (*queue)->id);
...
hsa_status_t hsa_queue_destroy(hsa_queue_t *queue)
{
    ...
    tracepoint(hsa_runtime, queue_destroyed, "queue_profiling", queue_id);
