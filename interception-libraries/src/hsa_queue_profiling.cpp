...
hsa_status_t aql_callback(const hsa_aql_trace_t *aql_trace, void *user_arg)
{
    switch (aql_trace->type) {
        case HSA_PACKET_TYPE_VENDOR_SPECIFIC:
            tracepoint(interceptionTracer, aql_packet_submitted, "queue_profiling", aql_trace->packet_id,
        ...
        case HSA_PACKET_TYPE_KERNEL_DISPATCH: {
            ...
            tracepoint(interceptionTracer, aql_kernel_dispatch_packet_submitted,
    ...
    uint32_t group_segment_size, hsa_queue_t **queue)
{
    ...
    tracepoint(interceptionTracer, queue_created, "queue_profiling", agent.handle, (*queue)->id);
...
hsa_status_t hsa_queue_destroy(hsa_queue_t *queue)
{
    ...
    tracepoint(interceptionTracer, queue_destroyed, "queue_profiling", queue_id);
